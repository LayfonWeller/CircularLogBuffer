...
template <typename CirLogs> void testCircularLogs(CirLogs test = CirLogs())
{
  ...
  do {
    std::cout << " [" << i << "] => ";
    std::copy(std::make_reverse_iterator(test.end()), std::make_reverse_iterator(test.begin()),
              std::ostream_iterator<typename CirLogs::value_type>(std::cout, ", "));

    std::cout << " <==> ";

    std::copy(test.begin(), test.end(), std::ostream_iterator<typename CirLogs::value_type>(std::cout, ", "));
  ...
  std::cout << " [" << i << "] =>";
  std::copy(test.crbegin(), test.crend(), std::ostream_iterator<typename CirLogs::value_type>(std::cout, ", "));

  std::cout << " <==> ";

  std::copy(test.cbegin(), test.cend(), std::ostream_iterator<typename CirLogs::value_type>(std::cout, ", "));
