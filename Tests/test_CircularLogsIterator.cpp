...
void Quicksort(BedExitLogData arr[BED_EXIT_LOG_CAPACITY], const size_t low,
               const size_t high, const bool logFull)
{
    auto beg = BedExitLogIterator(arr, low, false);
    auto end = BedExitLogIterator(arr, high, logFull);
    std::stable_sort(beg, end,
                     [](const BedExitLogData &a, const BedExitLogData &b) {
...
static void SortLogs()
{
    ...
    if (bBedExitLogFull)
        Quicksort(sBedExitLogData, byBedExitLogIndex, byBedExitLogIndex,
                  bBedExitLogFull);
    else
...
void lin_actualSendBedExitLog()
{
    ...
    printf("[%d/%d] %d : %d\n", sLinBedExitLog.byDataNumber,
           sLinBedExitLog.byDataCount, sLinBedExitLog.sLogData.date,
